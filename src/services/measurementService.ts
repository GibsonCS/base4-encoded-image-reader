...
import type { ConfirmMeasurement, Measurement } from '../schemas/measurementSchema.js';
...
export class MeasurementService {
  ...
  getMeasurement = async (measureData: Measurement) => {
    const { image, measure_datetime, measure_type, customer_code } = measureData;

    const imageId: string = crypto.randomUUID();
    await writeFile(`${process.cwd()}/src/images/${imageId}.jpeg`, Buffer.from(image, 'base64'));
    ...
    const measurements = this.measurementRepository.findByCustomerCode(customer_code);
    const lastMeasurement = measurements[measurements.length - 1];

    const isWithin30Days = (lastMeasurement: string, currentMeasurement: string) => {
      const lastMeasurementTimeStemp = new Date(lastMeasurement).valueOf();
      const todayLess30Days = dayjs(currentMeasurement).subtract(30, 'day').valueOf();
    ...
    if (isWithin30Days(lastMeasurement.measure_datetime, measure_datetime)) return DOUBLE_REPORT;
  ...
  getAllMeasurementByCustomerCode = async (customerCode: string, measureType?: string) => {
    const result = await this.measurementRepository.findByCustomerCodeAndMeasureType(
      customerCode,
